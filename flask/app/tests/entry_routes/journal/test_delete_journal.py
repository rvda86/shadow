...
# /api/entry/journal DELETE
class TestDeleteJournal(unittest.TestCase):
    db = db_pool.acquire()
    ...
    def test_success(self):
        data, status_code = self.requester.delete_entry("journal", self.journal["entry"]["id"], self.token_1)
        self.assertEqual(200, status_code)

    def test_combination_journal_author_unknown(self):
        data, status_code = self.requester.delete_entry("journal", self.journal["entry"]["id"], self.token_2)
        self.assertEqual(404, status_code)

    def test_invalid_id(self):
        invalid_id = "invalidID"
        data, status_code = self.requester.delete_entry("journal", invalid_id, self.token_1)
