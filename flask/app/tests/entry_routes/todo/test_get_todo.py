...
class TestGetToDo(unittest.TestCase):
    ...
    def test_success(self):
        data, status_code = self.requester.get_entry("todo", self.todo["entry"]["id"], self.token_1)
        self.assertEqual(200, status_code)

    def test_combination_todo_author_unknown(self):
        data, status_code = self.requester.get_entry("todo", self.todo["entry"]["id"], self.token_2)
        self.assertEqual(404, status_code)

    def test_todo_does_not_exist(self):
        todo_id = uuid_generator()
        data, status_code = self.requester.get_entry("todo", todo_id, self.token_1)
        self.assertEqual(404, status_code)

    def test_invalid_id(self):
        invalid_id = "invalidID"
        data, status_code = self.requester.get_entry("todo", invalid_id, self.token_1)
